...
Bar::Bar(int width, int height, COLORREF borderColor, COLORREF innerColor, COLORREF backgroundColor, Point point) {
    this->width = width;
    this->height = height;
...
void Bar::Show(int value, int maxValue) {
    ...
    game_framework::Draw::Instance()->Rectangle(point + Point(2, 1), point + Point(width, height) + Point(-1, -1), backgroundColor);
}
